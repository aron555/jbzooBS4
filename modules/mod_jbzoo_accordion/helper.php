    ...
    public function render($category, $params, $flat, $level = 0, $attribs = null, $expanded = false)
    {
        ...
        foreach ($category->getChildren() as $child) {
            ...
            if ($flat == "dropdown") {
                if ($parent) {
                    ?>
                    <li class="dropdown-item dropdown-submenu parent <?= ($active ? ' active' : '') . ($current ? ' current' : '') ?>">
                        <div class="inner position-relative <?= ($active ? ' active' : '') . ($current ? ' current' : '') ?>">
                    ...
                    ?>
                    <li class="dropdown-item parent <?= ($active ? ' active' : '') . ($current ? ' current' : '') ?>">
                        <a
                                href="<?= $url ?>"
                                class=""
                        >
            ...
            if ($flat == "megamenu") {
                ...
                if ($expanded == false) {
                    if ($parent) {
                        ...
                        <a
                                href="<?= $url ?>"
                                class="parent smuzi-megamenu-link level<?= $nextLevel ?> d-block<?= ($active ? ' active' : '') . ($current ? ' current' : '') ?>"
                                data-list="zoo-list-<?= $child->alias ?>"
                        >
                            <?= $child->name ?>
                        </a>
                        <?php
                        ...
                        <a
                                href="<?= $url ?>"
                                class="notparent level<?= $nextLevel ?> d-block<?= ($active ? ' active' : '') . ($current ? ' current' : '') ?>"
                        >
                            <?= $child->name ?>
                        </a>
                        <?php
                ...
                if ($expanded == true) {
                    if ($parent) {
                        ...
                        $this->render($child, $params, $flat, $nextLevel, 'id="zoo-list-' . $child->alias . '" class="zoo-list-megamenu d-none white z-depth-1 level' . $nextLevel . '"', true);
