...
class JBHtmlHelper extends AppHelper
{
    ...
    public function quantity($default = 1, $options = array(), $id = null, $name = 'quantity', $return = false)
    {
        ...
        $html = array(
            '<div class="jsQuantity quantity-block d-flex flex-row" id="' . $id . '">',
                '<div class=""><span class="jsRemove minus_field btn btn-primary">–</span></div>',
                '<div class="jsCountBox">',
                    '<dl class="item-count-digits d-none">' . str_repeat('<dd></dd>', 5) . '</dl>',
                    '<span class="item-count">',
                        '<input type="text" value="' . $default . '" style="opacity: 1;" class="jsInput text-center quantity_field form-control" name="' . $name . '">',
                    '</span>',
                '</div>',
                '<div class=""><span class="jsAdd plus_field btn btn-primary">+</span></div>',
            '</div>'
        );
    ...
    public function dataList($data)
    {
        ...
        if (!empty($data)) {
            ...
            $html[] = '<dl class="row param-list">';

            foreach ($data as $label => $text) {
                $label  = JText::_($label);
                $html[] = '<dt class="col-sm-3" title="' . $this->cleanAttrValue($label) . '">' . $label . '</dt>';
                $html[] = '<dd class="col-sm-9">' . $text . '</dd>';
            }
    ...
    protected function _buildAttrs($attrs, $clean = true)
    {
        ...
        return \Joomla\String\StringHelper::trim($result);
    }
